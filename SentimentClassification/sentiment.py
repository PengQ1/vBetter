...
def load_test_data(filepath, input_shape=20):
    ...
    # 字典列表
    word_dictionary = {word: i+1 for i, word in enumerate(vocabulary)}
    with open('word_dict_test.pk', 'wb') as f:
        pickle.dump(word_dictionary, f)
    inverse_word_dictionary = {i+1: word for i, word in enumerate(vocabulary)}
    label_dictionary = {label: i for i, label in enumerate(labels)}
    with open('label_dict_test.pk', 'wb') as f:
        pickle.dump(label_dictionary, f)
    output_dictionary = {i: labels for i, labels in enumerate(labels)}
...
def model_train(input_shape, filepath, model_save_path):
    ...
    # input_shape = 100
    train_x, train_y, output_dictionary, vocab_size, label_size, inverse_word_dictionary = load_data(filepath, input_shape)
    test_x, test_y, output_dictionary_test, vocab_size_test, label_size_test, inverse_word_dictionary_test = load_test_data('./test.csv', input_shape)
    ...
    for start, end in zip(range(0, N, 1), range(1, N+1, 1)):
        sentence = [inverse_word_dictionary_test[i] for i in test_x[start] if i != 0]
        y_predict = lstm_model.predict(test_x[start:end])
        label_predict = output_dictionary[np.argmax(y_predict[0])]
        #label_true = output_dictionary[np.argmax(test_y[start:end])]
        print(''.join(sentence), label_predict) # 输出预测结果
